...
export default function FindCompany() {
  ...
  const options = [
    { value: 'ueid', label: 'Unique Entity ID (UEID)', placeholder: 'Enter UEID...' },
    { value: 'cage', label: 'CAGE Code', placeholder: 'Enter CAGE Code...' },
    { value: 'companyName', label: 'Company Name', placeholder: 'Choose from list' },
  ];
  ...
  return (
    <AuthLayout showBack={false}>
      <div className="flex flex-col justify-center min-h-[calc(100vh-10rem)] w-full max-w-xl mx-auto">
        ...
        <h1 className="text-4xl font-bold mb-3 text-gray-900">Find your company</h1>
        <p className="text-base text-gray-500 mb-10 font-medium">
          Search for your company below. If it's not found, please{' '}
          <Link to="/onboarding/company-info" className="text-iwePrimary font-bold hover:underline">Add it now</Link>
        </p>

        <form onSubmit={(e) => { e.preventDefault(); navigate('/onboarding/company-info'); }}>
          <RadioGroup value={searchMethod} onValueChange={setSearchMethod} className="space-y-8 mb-12 w-full">
            {options.map((opt) => (
              <div key={opt.value} className="space-y-4">
                <div className="flex items-center space-x-4">
                  <RadioGroupItem value={opt.value} id={opt.value} className="scale-125 origin-left" />
                  <Label htmlFor={opt.value} className="text-lg font-semibold text-gray-800 cursor-pointer">
                    {opt.label}
                  </Label>
                </div>
                ...
                {searchMethod === opt.value && (
                  <div className="pl-9">
                    {opt.value === 'companyName' ? (
                      <Select onValueChange={(val) => setForm({ ...form, companyName: val })}>
                        ...
                        <SelectContent className="bg-white border-gray-200 shadow-xl z-50">
                          <SelectItem value="Cencore LLC" className="text-base focus:bg-gray-100 focus:text-iwePrimary py-3 cursor-pointer">Cencore LLC</SelectItem>
                          <SelectItem value="Encore" className="text-base focus:bg-gray-100 focus:text-iwePrimary py-3 cursor-pointer">Encore</SelectItem>
                        </SelectContent>
                      </Select>
                    ) : (
                      <Input
                        value={form[opt.value]}
                        onChange={(e) => setForm({ ...form, [opt.value]: e.target.value })}
          ...
          <Button
            type="submit" disabled={!form[searchMethod]}
